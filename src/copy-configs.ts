...
const DONT_COPY = new Set(["package.json", "README.md", "LICENSE"]);

async function isConfigForTarget(src: string): Promise<boolean> {
	const stats = await lstat(src);
	const isNonRootDirectory =
		stats.isDirectory() && src !== getTemplatePath(".");
...
export default async function copyConfigs(answers?: Answers) {
	...
	await Promise.all([
		cp(getTemplatePath("."), ".", {
			recursive: true,
			filter: isConfigForTarget,
			force: false,
		}),
		cp(
			getTemplatePath(".github/workflows/base-build-and-test.yml"),
			".github/workflows/base-build-and-test.yml",
			{ force: false },
		),
		cp(
			getTemplatePath(sourceGitHubWorkflowPath),
			join(".github/workflows", basename(sourceGitHubWorkflowPath)),
			{ force: false },
		),
		cp(getTemplatePath(".husky"), ".husky", { recursive: true, force: true }),
		cp(getTemplatePath("config/tsm"), "config/tsm", {
			recursive: true,
			force: true,
		}),
	]);
