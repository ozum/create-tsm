...
const SOURCE_ONLY_DEV_DEPS = new Set([
	"type-fest",
	"@types/inquirer",
...
function getScripts(scripts: PackageJson.Scripts = {}): {
	prefixedScripts: PackageJson.Scripts;
	targetedScripts: PackageJson.Scripts;
} {
	const prefixedScripts = transformObject(scripts, {
		filter: (k) => k.startsWith("base:"),
	});
	const targetedScripts = transformObject(prefixedScripts, {
		transform: (k) => [k.replace("base:", ""), `pnpm ${k}`],
	});
...
async function createPackageJson(answers: Answers): Promise<boolean> {
	const sourcePackage = JSON.parse(
		await getTemplateContent("package.json"),
	) as PackageJson;
	const { prefixedScripts, targetedScripts } = getScripts(
	...
	const pkg = {
		...
		bugs: {
			url: answers.gitUser
				? `https://github.com/${answers.gitUser.username}/${answers.repoName}/issues`
				: "",
		},
		homepage: answers.gitUser
			? `https://github.com/${answers.gitUser.username}/${answers.repoName}`
			: "",
...
async function updatePackageJson(): Promise<boolean> {
	...
	const isDepsEqual =
		...
		deepEqual(
			targetPackageBefore.devDependencies,
			targetPackage.devDependencies,
		) &&
		deepEqual(
			targetPackageBefore.peerDependencies,
			targetPackage.peerDependencies,
		) &&
		deepEqual(
			targetPackageBefore.optionalDependencies,
			targetPackage.optionalDependencies,
		);

	if (!isEqual)
		await writePrettyFile("package.json", JSON.stringify(targetPackage), true);
	return !isDepsEqual;
}
...
export async function createOrUpdatePackageJson(
	...
	if (answers === undefined)
		throw new Error("Answers are required to create package.json");
